...
const currentSearch = handleAction(changeSearch, (state, action) => action.payload, '')

const searchResult = handleAction(
  searchResultReceived,
  (state, action) =>
    reject(isNil, [action.payload.data.codepoint, ...(action.payload.data.codepointSearch || [])]),
  [],
)
